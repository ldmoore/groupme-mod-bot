...
test("free cash giveaway", () => {
	expect(containsBlockedPhrase("Join our free cash giveaway now!")).toBe(true);
});

test("no blocked phrase", () => {
	expect(containsBlockedPhrase("This message is clean and safe.")).toBe(false);
});
...
test("verify your account", () => {
	expect(containsBlockedPhrase("Please verify your account to continue.")).toBe(
		true,
	);
});

test("clean message", () => {
	expect(containsBlockedPhrase("This message is safe and clean.")).toBe(false);
});
